...
{
    public class App3DbContext : DbContext, IApp3DbContext
    {
        // TODO(crhodes)
        ...
        public App3DbContext() : base("App3_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<App3DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<App3DbContext>(new CreateDatabaseIfNotExists<App3DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<App3DbContext>(new DropCreateDatabaseIfModelChanges<App3DbContext>());

            Database.SetInitializer<App3DbContext>(
                new DropCreateDatabaseIfModelChanges<App3DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<App3DbContext>(new DropCreateDatabaseAlways<App3DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<App3DbContext>(new App3DbContextDatabaseInitializer());
