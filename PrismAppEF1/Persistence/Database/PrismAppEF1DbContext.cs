...
{
    public class PrismAppEF1DbContext : DbContext, IPrismAppEF1DbContext
    {
        // TODO(crhodes)
        ...
        public DbSet<Song> SongsSet { get; set; }

        // Name of connection string in Config

        public PrismAppEF1DbContext() : base("PrismAppEF1_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PrismAppEF1DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PrismAppEF1DbContext>(new CreateDatabaseIfNotExists<PrismAppEF1DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PrismAppEF1DbContext>(new DropCreateDatabaseIfModelChanges<PrismAppEF1DbContext>());

            Database.SetInitializer<PrismAppEF1DbContext>(
                new DropCreateDatabaseIfModelChanges<PrismAppEF1DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PrismAppEF1DbContext>(new DropCreateDatabaseAlways<PrismAppEF1DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PrismAppEF1DbContext>(new PrismAppEF1DbContextDatabaseInitializer());
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                ...
                Log.PERSISTENCE($"(PrismAppEF1DbContext) Exit ({result})", Common.LOG_APPNAME, startTicks);
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("(PrismAppEF1DbContext) Enter", Common.LOG_APPNAME);
