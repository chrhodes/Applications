...
{
    public class TestPrismApp2DbContext : DbContext, ITestPrismApp2DbContext
    {
        public DbSet<Dog> DogSet { get; set; }

        // Name of connection string in Config

        public TestPrismApp2DbContext() : base("TestPrismApp2_DB")
        {
            Int64 startTicks = Log.Trace(String.Format("Enter"), Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<TestPrismApp2DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<TestPrismApp2DbContext>(new CreateDatabaseIfNotExists<TestPrismApp2DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<TestPrismApp2DbContext>(new DropCreateDatabaseIfModelChanges<TestPrismApp2DbContext>());

            Database.SetInitializer<TestPrismApp2DbContext>(
                new DropCreateDatabaseIfModelChanges<TestPrismApp2DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<TestPrismApp2DbContext>(new DropCreateDatabaseAlways<TestPrismApp2DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<TestPrismApp2DbContext>(new TestPrismApp2DbContextDatabaseInitializer());
