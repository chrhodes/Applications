...
{
    public class PrismApp3DbContext : DbContext, IPrismApp3DbContext
    {
        // TODO(crhodes)
        ...
        public PrismApp3DbContext() : base("PrismApp3_DB")
        {
            Int64 startTicks = Log.PERSISTENCE("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PrismApp3DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PrismApp3DbContext>(new CreateDatabaseIfNotExists<PrismApp3DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PrismApp3DbContext>(new DropCreateDatabaseIfModelChanges<PrismApp3DbContext>());

            Database.SetInitializer<PrismApp3DbContext>(
                new DropCreateDatabaseIfModelChanges<PrismApp3DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PrismApp3DbContext>(new DropCreateDatabaseAlways<PrismApp3DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PrismApp3DbContext>(new PrismApp3DbContextDatabaseInitializer());
