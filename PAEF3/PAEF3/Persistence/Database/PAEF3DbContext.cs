...
{
    public class PAEF3DbContext : DbContext, IPAEF3DbContext
    {
        // TODO(crhodes)
        ...
        public PAEF3DbContext() : base("PAEF3_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_CATEGORY);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PAEF3DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PAEF3DbContext>(new CreateDatabaseIfNotExists<PAEF3DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PAEF3DbContext>(new DropCreateDatabaseIfModelChanges<PAEF3DbContext>());

            Database.SetInitializer<PAEF3DbContext>(
                new DropCreateDatabaseIfModelChanges<PAEF3DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PAEF3DbContext>(new DropCreateDatabaseAlways<PAEF3DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PAEF3DbContext>(new PAEF3DbContextDatabaseInitializer());
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                ...
                Log.PERSISTENCE($"(PAEF3DbContext) Exit ({result})", Common.LOG_CATEGORY, startTicks);
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("(PAEF3DbContext) Enter", Common.LOG_CATEGORY);
