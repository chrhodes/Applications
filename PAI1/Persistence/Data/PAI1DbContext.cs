...
{
    public class PAI1DbContext : DbContext, IPAI1DbContext
    {
        // TODO(crhodes)
        ...
        public PAI1DbContext() : base("PAI1_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PAI1DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PAI1DbContext>(new CreateDatabaseIfNotExists<PAI1DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PAI1DbContext>(new DropCreateDatabaseIfModelChanges<PAI1DbContext>());

            Database.SetInitializer<PAI1DbContext>(
                new DropCreateDatabaseIfModelChanges<PAI1DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PAI1DbContext>(new DropCreateDatabaseAlways<PAI1DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PAI1DbContext>(new PAI1DbContextDatabaseInitializer());
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("(PAI1DbContext) Enter", Common.LOG_APPNAME);
            ...
            Log.PERSISTENCE($"(PAI1DbContext) Exit ({result})", Common.LOG_APPNAME, startTicks);
