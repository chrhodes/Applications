...
{
    public class PrismApp2DbContext : DbContext, IPrismApp2DbContext
    {
        // TODO(crhodes)
        ...
        public PrismApp2DbContext() : base("PrismApp2_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PrismApp2DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PrismApp2DbContext>(new CreateDatabaseIfNotExists<PrismApp2DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PrismApp2DbContext>(new DropCreateDatabaseIfModelChanges<PrismApp2DbContext>());

            Database.SetInitializer<PrismApp2DbContext>(
                new DropCreateDatabaseIfModelChanges<PrismApp2DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PrismApp2DbContext>(new DropCreateDatabaseAlways<PrismApp2DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PrismApp2DbContext>(new PrismApp2DbContextDatabaseInitializer());
