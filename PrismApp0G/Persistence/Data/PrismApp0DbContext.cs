...
{
    public class PrismApp0DbContext : DbContext, IPrismApp0DbContext
    {
        // TODO(crhodes)
        ...
        public PrismApp0DbContext() : base("PrismApp0_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PrismApp0DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PrismApp0DbContext>(new CreateDatabaseIfNotExists<PrismApp0DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PrismApp0DbContext>(new DropCreateDatabaseIfModelChanges<PrismApp0DbContext>());

            Database.SetInitializer<PrismApp0DbContext>(
                new DropCreateDatabaseIfModelChanges<PrismApp0DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PrismApp0DbContext>(new DropCreateDatabaseAlways<PrismApp0DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PrismApp0DbContext>(new PrismApp0DbContextDatabaseInitializer());
