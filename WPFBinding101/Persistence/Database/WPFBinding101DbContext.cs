...
{
    public class WPFBinding101DbContext : DbContext, IWPFBinding101DbContext
    {
        // TODO(crhodes)
        // Add additional DbSet<TYPE> as needed.

        public DbSet<Bird> BirdsSet { get; set; }
        public DbSet<BirdPhoneNumber> BirdPhoneNumbersSet { get; set; }

        public DbSet<Seed> SeedsSet { get; set; }

        // Name of connection string in Config

        public WPFBinding101DbContext() : base("WPFBinding101_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<WPFBinding101DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<WPFBinding101DbContext>(new CreateDatabaseIfNotExists<WPFBinding101DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<WPFBinding101DbContext>(new DropCreateDatabaseIfModelChanges<WPFBinding101DbContext>());

            Database.SetInitializer<WPFBinding101DbContext>(
                new DropCreateDatabaseIfModelChanges<WPFBinding101DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<WPFBinding101DbContext>(new DropCreateDatabaseAlways<WPFBinding101DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<WPFBinding101DbContext>(new WPFBinding101DbContextDatabaseInitializer());
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                ...
                Log.PERSISTENCE($"(WPFBinding101DbContext) Exit ({result})", Common.LOG_APPNAME, startTicks);
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("(WPFBinding101DbContext) Enter", Common.LOG_APPNAME);
