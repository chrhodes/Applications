...
{
    public class PAEF1aDbContext : DbContext, IPAEF1aDbContext
    {
        // TODO(crhodes)
        ...
        public PAEF1aDbContext() : base("PAEF1a_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_CATEGORY);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PAEF1aDbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PAEF1aDbContext>(new CreateDatabaseIfNotExists<PAEF1aDbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PAEF1aDbContext>(new DropCreateDatabaseIfModelChanges<PAEF1aDbContext>());

            Database.SetInitializer<PAEF1aDbContext>(
                new DropCreateDatabaseIfModelChanges<PAEF1aDbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PAEF1aDbContext>(new DropCreateDatabaseAlways<PAEF1aDbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PAEF1aDbContext>(new PAEF1aDbContextDatabaseInitializer());
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                ...
                Log.PERSISTENCE($"(PAEF1aDbContext) Exit ({result})", Common.LOG_CATEGORY, startTicks);
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("(PAEF1aDbContext) Enter", Common.LOG_CATEGORY);
