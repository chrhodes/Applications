...
{
    public class TestPrismApp1DbContext : DbContext, ITestPrismApp1DbContext
    {
        public DbSet<Dog> DogSet { get; set; }

        // Name of connection string in Config

        public TestPrismApp1DbContext() : base("TestPrismApp1_DB")
        {
            Int64 startTicks = Log.Trace(String.Format("Enter"), Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<TestPrismApp1DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<TestPrismApp1DbContext>(new CreateDatabaseIfNotExists<TestPrismApp1DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<TestPrismApp1DbContext>(new DropCreateDatabaseIfModelChanges<TestPrismApp1DbContext>());

            Database.SetInitializer<TestPrismApp1DbContext>(
                new DropCreateDatabaseIfModelChanges<TestPrismApp1DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<TestPrismApp1DbContext>(new DropCreateDatabaseAlways<TestPrismApp1DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<TestPrismApp1DbContext>(new TestPrismApp1DbContextDatabaseInitializer());
