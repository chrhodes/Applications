...
{
    public class PrismEFI2DbContext : DbContext, IPrismEFI2DbContext
    {
        // TODO(crhodes)
        ...
        public PrismEFI2DbContext() : base("PrismEFI2_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_APPNAME);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<PrismEFI2DbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<PrismEFI2DbContext>(new CreateDatabaseIfNotExists<PrismEFI2DbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<PrismEFI2DbContext>(new DropCreateDatabaseIfModelChanges<PrismEFI2DbContext>());

            Database.SetInitializer<PrismEFI2DbContext>(
                new DropCreateDatabaseIfModelChanges<PrismEFI2DbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<PrismEFI2DbContext>(new DropCreateDatabaseAlways<PrismEFI2DbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<PrismEFI2DbContext>(new PrismEFI2DbContextDatabaseInitializer());
